...
class HorizontalControl:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Compute cascade PID control
            vx_control = self.pid_x.compute(self.x_sp, self.x_mv)
            pitch_sp_inertial = self.pid_vx.compute(vx_control, self.vx_mv)
            
            vy_control = self.pid_y.compute(self.y_sp, self.y_mv)
            roll_sp_inertial = -self.pid_vy.compute(vy_control, self.vy_mv)
            
            # Transform roll and pitch setpoints to body frame
            roll_sp = math.cos(self.yaw_mv) * roll_sp_inertial + math.sin(self.yaw_mv) * pitch_sp_inertial
            pitch_sp = -math.sin(self.yaw_mv) * roll_sp_inertial + math.cos(self.yaw_mv) * pitch_sp_inertial
    ...
    def transform_velocity_errors(self, vx_error, vy_error):
        '''
        Transforms velocity errors from the inertial frame to the body frame.
        '''
        cos_yaw = math.cos(self.yaw_mv)
        sin_yaw = math.sin(self.yaw_mv)
        vx_error_body = cos_yaw * vx_error + sin_yaw * vy_error
        vy_error_body = -sin_yaw * vx_error + cos_yaw * vy_error
        return vx_error_body, vy_error_body
