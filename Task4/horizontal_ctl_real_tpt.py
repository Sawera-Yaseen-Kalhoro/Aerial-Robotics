    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.joyMasterFlag == False:
                ...
                # Compute control for vx (velocity in x direction)
                vx_control = self.pid_x.compute(self.x_sp, self.x_mv)  
                pitch_sp = self.pid_vx.compute(vx_control, self.vx_mv)  
                
                # Compute control for vy (velocity in y direction)
                vy_control = self.pid_y.compute(self.y_sp, self.y_mv)  
                roll_sp = -self.pid_vy.compute(vy_control, self.vy_mv)  
