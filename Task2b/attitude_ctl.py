...
class AttitudeControl:
    ...
    def __init__(self):
        ...
        self.pid_roll_rate.set_kp(60)
        self.pid_roll_rate.set_ki(10)
        self.pid_roll_rate.set_kd(0)

        self.pid_roll.set_kp(10)
        self.pid_roll.set_ki(2)
        self.pid_roll.set_kd(0)

        self.pid_pitch_rate.set_kp(70)
        self.pid_pitch_rate.set_ki(10)
        self.pid_pitch_rate.set_kd(0)

        self.pid_pitch.set_kp(10)
        self.pid_pitch.set_ki(2)
        self.pid_pitch.set_kd(0)

        self.pid_yaw_rate.set_kp(2)
        self.pid_yaw_rate.set_ki(0)
        self.pid_yaw_rate.set_kd(0)
        
        self.pid_yaw.set_kp(8)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            roll_rate_sp = self.pid_roll.compute(self.euler_sp.x, self.euler_mv.x)
            roll_torque = self.pid_roll_rate.compute(roll_rate_sp, self.euler_rate_mv.x)

            pitch_rate_sp = self.pid_pitch.compute(self.euler_sp.y, self.euler_mv.y)
            pitch_torque = self.pid_pitch_rate.compute(pitch_rate_sp, self.euler_rate_mv.y)

            yaw_rate_sp = self.pid_yaw.compute(self.euler_sp.z, self.euler_mv.z)
            yaw_torque = self.pid_yaw_rate.compute(yaw_rate_sp, self.euler_rate_mv.z)

            mot_sp1 = self.w_sp - roll_torque - pitch_torque - yaw_torque
            mot_sp2 = self.w_sp + roll_torque - pitch_torque + yaw_torque
            mot_sp3 = self.w_sp + roll_torque + pitch_torque - yaw_torque
            mot_sp4 = self.w_sp - roll_torque + pitch_torque + yaw_torque
